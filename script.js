...
function render() {
  ...
  DATA.forEach((section, sIdx) => {
    ...
    const checked = section.items.filter((_, iIdx) => state[itemId(sIdx, iIdx)]).length;
    totalAll += total;
    checkedAll += checked;
    ...
    section.items.forEach((item, iIdx) => {
      ...
      row.className = 'item' + (item.sub ? ' sub' : '') + (isChecked ? ' checked' : '');
  ...
  document.getElementById('overall-num').textContent = `${checkedAll} / ${totalAll}`;
